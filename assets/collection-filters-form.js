...
class PriceRange extends HTMLElement {
  ...
  activeSwatch() {
   ...
   if (url.searchParams.has('filter.v.option.color')) {
    window.setTimeout(function() {
      let itemSwatchSlt = document.querySelectorAll('.product-collection .product-bottom .item-swatch-custom.active');

        let itemSwatchSltLength = itemSwatchSlt.length;

          for (let i=0; i<itemSwatchSltLength; i++) {
              const thisSwatch = itemSwatchSlt[i];

              const thisItem = thisSwatch.closest('.product-item');
              const thisImage = thisItem.querySelector('.product-grid-image > [data-index="0"] > img');
              const thisSource = thisItem.querySelector('.product-grid-image > [data-index="0"] > .image-source-1');
              const thisSource2 = thisItem.querySelector('.product-grid-image > [data-index="0"] > .image-source-2');
              const imgActive = thisSwatch.getAttribute('data-img');
        
              if (imgActive != '' && imgActive != null) {
                thisImage.setAttribute('srcset', imgActive);
